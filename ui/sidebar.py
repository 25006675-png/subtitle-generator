...
class Sidebar(ctk.CTkFrame):
    def __init__(self, parent, state, **kwargs):
        ...
        # Accent dot + brand name
        self.brand_inner = ctk.CTkFrame(self.brand_frame, fg_color="transparent")
        self.brand_inner.grid(row=0, column=1, sticky="w", padx=(SPACING["xs"], 0))

        ff = get_font_family()
        self.accent_dot = ctk.CTkLabel(
            self.brand_inner, text="\u25CF",
            font=ctk.CTkFont(size=12),
            text_color=COLORS["accent"],
            width=12,
        )
        self.accent_dot.grid(row=0, column=0, padx=(0, SPACING["xs"]))
