...
class StylePanel(ctk.CTkFrame):
    def __init__(self, parent, state, **kwargs):
        ...
        scope_frame.grid(row=1, column=0, columnspan=2, sticky="ew",
        ...
        ctk.CTkLabel(
            ...
            anchor="w",
        ).grid(row=0, column=0, sticky="w", padx=SPACING["lg"], pady=(SPACING["md"], SPACING["xs"]))
        ...
        self.scope_btn.grid(row=1, column=0, sticky="ew", padx=SPACING["lg"], pady=(0, SPACING["md"]))

        # --- Preset Section ---
        preset_frame = ctk.CTkFrame(self, fg_color=COLORS["bg_secondary"], corner_radius=RADIUS["card"])
        preset_frame.grid(row=2, column=0, columnspan=2, sticky="ew",
                          padx=SPACING["lg"], pady=(0, SPACING["md"]))
        ...
        anim_card.grid(row=3, column=0, columnspan=2, sticky="ew",
                       padx=SPACING["lg"], pady=(0, SPACING["md"]))
        ...
        self.classic_settings.grid_columnconfigure(1, weight=1)
        ...
        ctk.CTkLabel(
            self.classic_settings, text="Unspoken:",
            font=ctk.CTkFont(family=ff, size=FONTS["body"][1]),
            text_color=COLORS["text_secondary"],
        ...
        self.popup_trail_slider.grid(row=0, column=1, sticky="ew")
        ...
        self.popup_trail_val.grid(row=0, column=2, padx=(SPACING["xs"], SPACING["lg"]))

        ctk.CTkLabel(
            self.popup_settings, text="Minimum Characters:",
            font=ctk.CTkFont(family=ff, size=FONTS["body"][1]),
            text_color=COLORS["text_secondary"],
        ).grid(row=1, column=0, padx=(SPACING["lg"], SPACING["md"]), pady=(SPACING["xs"], 0))

        self.popup_min_chars_slider = ctk.CTkSlider(
            self.popup_settings, from_=0, to=8, number_of_steps=8,
            progress_color=COLORS["accent"], button_color=COLORS["accent"],
            button_hover_color=COLORS["accent_hover"], fg_color=COLORS["progress_bg"],
            command=self._on_popup_min_chars_change, height=14,
        )
        self.popup_min_chars_slider.set(getattr(state, 'popup_min_chars', 3))
        self.popup_min_chars_slider.grid(row=1, column=1, sticky="ew", pady=(SPACING["xs"], 0))

        self.popup_min_chars_val = ctk.CTkLabel(
            self.popup_settings,
            text=str(getattr(state, 'popup_min_chars', 3)),
            font=ctk.CTkFont(family=FONTS["mono_small"][0], size=FONTS["mono_small"][1]),
            text_color=COLORS["text_secondary"], width=40,
        )
        self.popup_min_chars_val.grid(row=1, column=2, padx=(SPACING["xs"], SPACING["lg"]),
                                      pady=(SPACING["xs"], 0))
        ...
        ctk.CTkLabel(
            self.wordbyw_settings, text="History Opacity:",
            font=ctk.CTkFont(family=ff, size=FONTS["body"][1]),
            text_color=COLORS["text_secondary"],
        ).grid(row=1, column=0, padx=(SPACING["lg"], SPACING["md"]), pady=(SPACING["xs"], SPACING["xs"]))

        self.wordbyw_dim_slider = ctk.CTkSlider(
            self.wordbyw_settings, from_=0.0, to=1.0, number_of_steps=20,
            progress_color=COLORS["accent"], button_color=COLORS["accent"],
            button_hover_color=COLORS["accent_hover"], fg_color=COLORS["progress_bg"],
            command=self._on_wordbyw_dim_change, height=14,
        )
        self.wordbyw_dim_slider.set(getattr(state, "wordbyw_history_dim", 1.0))
        self.wordbyw_dim_slider.grid(row=1, column=1, sticky="ew",
                                     padx=(0, SPACING["xs"]), pady=(SPACING["xs"], SPACING["xs"]))

        self.wordbyw_dim_val = ctk.CTkLabel(
            self.wordbyw_settings,
            text=f"{getattr(state, 'wordbyw_history_dim', 1.0):.0%}",
            font=ctk.CTkFont(family=FONTS["mono_small"][0], size=FONTS["mono_small"][1]),
            text_color=COLORS["text_secondary"], width=36,
        )
        self.wordbyw_dim_val.grid(row=1, column=2, padx=(SPACING["xs"], SPACING["lg"]),
        ...
        self.trans_anim_card = ctk.CTkFrame(self, fg_color=COLORS["bg_secondary"], corner_radius=RADIUS["card"])
        self.trans_anim_card.grid(row=4, column=0, columnspan=2, sticky="ew",
                                   padx=SPACING["lg"], pady=(0, SPACING["md"]))
        ...
        self.style_container.grid(row=5, column=0, columnspan=2, sticky="nsew", padx=SPACING["lg"])
    ...
    def _on_dimmed_change(self, value):
        ...
        self._update_animation_settings(classic_dimmed_opacity=float(value))

    def _on_popup_trail_change(self, value):
        count = int(round(float(value)))
        self.popup_trail_val.configure(text=str(count))
        self._update_animation_settings(popup_trail_count=count)

    def _on_popup_min_chars_change(self, value):
        count = int(round(float(value)))
        self.popup_min_chars_val.configure(text=str(count))
        self._update_animation_settings(popup_min_chars=count)

    def _on_wordbyw_entry_change(self, value):
        self._update_animation_settings(wordbyw_entry_style=value.lower())

    def _on_wordbyw_dim_change(self, value):
        self.wordbyw_dim_val.configure(text=f"{float(value):.0%}")
        self._update_animation_settings(wordbyw_history_dim=float(value))

    def _on_active_marker_change(self, value):
        mapping = {"Color": "color", "Box": "box", "Color+Box": "color_box"}
        self._update_animation_settings(classic_active_marker=mapping.get(value, "color"))

    def _on_history_on_change(self):
        self._update_animation_settings(classic_history_on=self.history_on_switch.get() == 1)
    ...
    def _on_state_change(self, field):
        ...
        elif field == "popup_min_chars":
            self.popup_min_chars_slider.set(self.state.popup_min_chars)
            self.popup_min_chars_val.configure(text=str(self.state.popup_min_chars))
        elif field == "wordbyw_entry_style":
            self.wordbyw_entry_var.set(self.state.wordbyw_entry_style.capitalize())
        elif field == "wordbyw_history_dim":
            self.wordbyw_dim_slider.set(self.state.wordbyw_history_dim)
            self.wordbyw_dim_val.configure(text=f"{self.state.wordbyw_history_dim:.0%}")
    ...
    def _update_animation_settings(self, **kwargs):
        ...
        setter_map = {
            ...
            "classic_dimmed_opacity": self.state.set_classic_dimmed_opacity,
            "popup_trail_count": self.state.set_popup_trail_count,
            "popup_min_chars": self.state.set_popup_min_chars,
            "wordbyw_entry_style": self.state.set_wordbyw_entry_style,
            "wordbyw_history_dim": self.state.set_wordbyw_history_dim,
            "classic_active_marker": self.state.set_classic_active_marker,
            "classic_history_on": self.state.set_classic_history_on,
        }
        for key, value in kwargs.items():
    ...
    def _apply_animation_controls_from_scope(self):
        ...
        self.dimmed_slider.set(settings.classic_dimmed_opacity)
        self.dimmed_val_label.configure(text=f"{int(settings.classic_dimmed_opacity * 100)}%")
        self.active_marker_var.set({
            "color": "Color",
            "box": "Box",
            "color_box": "Color+Box",
        }.get(settings.classic_active_marker, "Color"))
        if settings.classic_history_on:
            self.history_on_switch.select()
        else:
            self.history_on_switch.deselect()
        self.popup_trail_slider.set(settings.popup_trail_count)
        self.popup_trail_val.configure(text=str(settings.popup_trail_count))
        self.popup_min_chars_slider.set(settings.popup_min_chars)
        self.popup_min_chars_val.configure(text=str(settings.popup_min_chars))
        self.wordbyw_entry_var.set({
            "instant": "Instant",
            "fade": "Fade",
            "pop": "Pop",
        }.get(settings.wordbyw_entry_style, "Instant"))
        self.wordbyw_dim_slider.set(settings.wordbyw_history_dim)
        self.wordbyw_dim_val.configure(text=f"{settings.wordbyw_history_dim:.0%}")
